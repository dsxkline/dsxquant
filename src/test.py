...
if __name__=="__main__":
    ...
    def closecallback(need_connect):
        logger.info("是否需要重连 %s" % need_connect)
        # 断线后重新连接
        asyncdemo()
    ...
    def asyncdemo():
        ...
        if dd_async:
            ...
            def quotes_all_callback(response:dsxquant.parser):
                ...
                # logger.info(dd)
                # 第一行是键名索引
                #  {'names': ['buy_1', 'buy_2', 'buy_3', 'buy_4', 'buy_5', 'buy_1_s', 'buy_2_s', 'buy_3_s', 'buy_4_s', 'buy_5_s', 'code', 'closePrice', 'cityNetRate', 'circulationValue', 'highPrice', 'isStop', 'lastDate', 'lastTime', 'lowPrice', 'name', 'openPrice', 'price', 'peRatio', 'swing', 'sell_1', 'sell_2', 'sell_3', 'sell_4', 'sell_5', 'sell_1_s', 'sell_2_s', 'sell_3_s', 'sell_4_s', 'sell_5_s', 'type', 'totalValue', 'turnoverRate', 'volumn', 'volumnPrice']}
                ...
                if "code" in names:
                    ...
                    logger.info("%s 笔 %s 时间 %s 当前时间 %s 延时 %s s" % (dd.__len__(),code,t,datetime.datetime.now(),s.seconds))
                else:
                    logger.debug(dd.values[0])
                    logger.info(dd.values[1])
                    pass
            # 订阅全市场所有股票实时行情
